...
| --- | --- |
| Summary | USB OTP VDD disruption can result in corrupt OTP row read data
_ _ |
| Affects | RP2350 A2 |
| Description | The OTP array has a read voltage generated from USB OTP VDD using an internal linear regulator. While the
_ _
regulator has a "power good" signal, it isn’t sampled outside of the initial power-on reset startup
sequence. External manipulation of USB OTP VDD can result in incorrect data being latched during the array
_ _
read phase.
