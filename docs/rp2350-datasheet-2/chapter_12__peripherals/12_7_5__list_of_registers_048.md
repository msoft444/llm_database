...
![Page 1160 figure](images/fig_p1160.png)
...
![Page 1161 figure](images/fig_p1161.png)
...
cleared. A corresponding bit in EP ABORT DONE is set when it is safe
_ _
to modify the buffer control register. |
| 0x064 | EP_ABORT_DONE | Device only: Used in conjunction with EP ABORT. Set once an
_
...
| 0x070 | EP_STATUS_STALL_NAK | Device: bits are set when the IRQ ON NAK or IRQ ON STALL bits are
_ _ _ _
set. For EP0 this comes from SIE CTRL. For all other endpoints it
_
...
![Page 1162 figure](images/fig_p1162.png)
...
the address of the device to communicate with. | RW | 0x00 |
...
![Page 1163 figure](images/fig_p1163.png)
...
Not isolated = 0, Isolated = 1 | RW | 0x1 |
...
![Page 1164 figure](images/fig_p1164.png)
...
![Page 1165 figure](images/fig_p1165.png)
...
* An IN packet from the device has the wrong data PID | WC | 0x0 |
...
not received in the maximum time specified by the USB spec. | WC | 0x0 |
| 26 | RX_OVERFLOW: RX overflow is raised by the Serial RX engine if the incoming
data is too fast. | WC | 0x0 |
...
and ep_tx_error registers to find out which endpoint had an error. | WC | 0x0 |
| 22:20 | Reserved. | - | - |
...
![Page 1166 figure](images/fig_p1166.png)
...
Raised by device if:
* An IN or OUT packet is sent with the LAST BUFF bit set in the buffer control
_
register
Raised by host if:
* A setup packet is sent when no data in or data out transaction follows * An
IN packet is received and the LAST BUFF bit is set in the buffer control register *
_
An IN packet is received with zero length * An OUT packet is sent and the
LAST BUFF bit is set
_ | WC | 0x0 |
...
registers. Host: the current transfer will be stopped early. | WC | 0x0 |
| 11 | RESUME: Host: Device has initiated a remote resume. Device: host has
initiated a resume. | WC | 0x0 |
...
device will go into suspend if neither Keep Alive / SOF frames are enabled. | RO | 0x0 |
...
![Page 1167 figure](images/fig_p1167.png)
...
![Page 1168 figure](images/fig_p1168.png)
...
![Page 1169 figure](images/fig_p1169.png)
...
![Page 1170 figure](images/fig_p1170.png)
...
![Page 1171 figure](images/fig_p1171.png)
...
![Page 1172 figure](images/fig_p1172.png)
...
![Page 1173 figure](images/fig_p1173.png)
...
This is done at a low level so overrides all other controls. | RW | 0x0 |
| 30:5 | Reserved. | - | - |
| 4 | USBPHY_AS_GPIO: Use the usb DP and DM pins as GPIO pins instead of
connecting them to the USB controller. | RW | 0x0 |
| 3 | SOFTCON | RW | 0x0 |
...
![Page 1174 figure](images/fig_p1174.png)
...
TX_DIFFMODE=1: Differential drive mode (TX_DM, TX_DM_OE ignored) | RW | 0x0 |
| 14 | TX_FSSLEW: TX_FSSLEW=0: Low speed slew rate
TX_FSSLEW=1: Full speed slew rate | RW | 0x0 |
...
If TX_DIFFMODE=0, OE for DPM only. 0 - DPM in Hi-Z state; 1 - DPM driving | RW | 0x0 |
| 8 | TX_DP_OE: Output enable. If TX_DIFFMODE=1, OE for DPP/DPM diff pair. 0 -
DPP/DPM in Hi-Z state; 1 - DPP/DPM driving
If TX_DIFFMODE=0, OE for DPP only. 0 - DPP in Hi-Z state; 1 - DPP driving | RW | 0x0 |
| 7 | Reserved. | - | - |
...
![Page 1175 figure](images/fig_p1175.png)
...
allows adjustment if required | RW | 0x1f |
| 7:5 | Reserved. | - | - |
...
![Page 1176 figure](images/fig_p1176.png)
...
qualified with a 1ms timer | RW | 0x1 |
| 6 | DEV_RX_ERR_QUIESCE: Device - suppress repeated errors until the device
FSM is next in the process of decoding an inbound packet. | RW | 0x1 |
...
accommodate worst-case hub delays. | RW | 0x0 |
...
before EOP. Only affects certain hubs. | RW | 0x0 |
...
host. Cleared by writing to SIE_STATUS.RESUME | RO | 0x0 |
...
![Page 1177 figure](images/fig_p1177.png)
...
writing to SIE_STATUS.SUSPENDED | RO | 0x0 |
| 13 | DEV_CONN_DIS: Set when the device connection state changes. Cleared by
writing to SIE_STATUS.CONNECTED | RO | 0x0 |
...
all bits in BUFF_STATUS. | RO | 0x0 |
| 3 | TRANS_COMPLETE: Raised every time SIE_STATUS.TRANS_COMPLETE is
set. Clear by writing to this bit. | RO | 0x0 |
...
writing to SIE_STATUS.RESUME | RO | 0x0 |
...
| 19 | EP_STALL_NAK: Raised when any bit in EP_STATUS_STALL_NAK is set. Clear
by clearing all bits in EP_STATUS_STALL_NAK. | RW | 0x0 |
| 18 | ABORT_DONE: Raised when any bit in ABORT_DONE is set. Clear by clearing
all bits in ABORT_DONE. | RW | 0x0 |
| 17 | DEV_SOF: Set every time the device receives a SOF (Start of Frame) packet.
Cleared by reading SOF_RD | RW | 0x0 |
...
![Page 1178 figure](images/fig_p1178.png)
...
| 15 | DEV_RESUME_FROM_HOST: Set when the device receives a resume from the
host. Cleared by writing to SIE_STATUS.RESUME | RW | 0x0 |
| 14 | DEV_SUSPEND: Set when the device suspend state changes. Cleared by
writing to SIE_STATUS.SUSPENDED | RW | 0x0 |
| 13 | DEV_CONN_DIS: Set when the device connection state changes. Cleared by
writing to SIE_STATUS.CONNECTED | RW | 0x0 |
...
| 4 | BUFF_STATUS: Raised when any bit in BUFF_STATUS is set. Clear by clearing
all bits in BUFF_STATUS. | RW | 0x0 |
| 3 | TRANS_COMPLETE: Raised every time SIE_STATUS.TRANS_COMPLETE is
set. Clear by writing to this bit. | RW | 0x0 |
| 2 | HOST_SOF: Host: raised every time the host sends a SOF (Start of Frame).
Cleared by reading SOF_RD | RW | 0x0 |
| 1 | HOST_RESUME: Host: raised when a device wakes up the host. Cleared by
writing to SIE_STATUS.RESUME | RW | 0x0 |
| 0 | HOST_CONN_DIS: Host: raised when a device is connected or disconnected
(i.e. when SIE_STATUS.SPEED changes). Cleared by writing to
SIE_STATUS.SPEED | RW | 0x0 |
...
![Page 1179 figure](images/fig_p1179.png)
...
![Page 1180 figure](images/fig_p1180.png)
...
![Page 1181 figure](images/fig_p1181.png)
...
calculate time between SOF events. | RO | 0x000000 |
...
![Page 1182 figure](images/fig_p1182.png)
