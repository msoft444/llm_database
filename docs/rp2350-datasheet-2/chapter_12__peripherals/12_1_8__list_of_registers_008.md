...
![Page 973 figure](images/fig_p0973.png)
...
![Page 974 figure](images/fig_p0974.png)
...
new character can be written to it. | RO | - |
...
valid start bit is received. | RO | - |
...
character at the top of the FIFO. | RO | - |
...
![Page 975 figure](images/fig_p0975.png)
...
associated with the character at the top of the FIFO. | RO | - |
...
In FIFO mode, this error is associated with the character at the top of the FIFO. | WC | 0x0 |
...
nUARTRI, modem status input. That is, the bit is 1 when nUARTRI is LOW. | RO | - |
...
![Page 976 figure](images/fig_p0976.png)
...
indicate if there is data in the transmit shift register. | RO | 0x1 |
...
when the receive FIFO is full. | RO | 0x0 |
...
when the transmit FIFO is full. | RO | 0x0 |
...
when the receive FIFO is empty. | RO | 0x1 |
...
becomes non-empty, regardless of whether the UART is enabled or not. | RO | 0x0 |
...
nUARTDSR, modem status input. That is, the bit is 1 when nUARTDSR is LOW. | RO | - |
| 0 | CTS: Clear to send. This bit is the complement of the UART clear to send,
nUARTCTS, modem status input. That is, the bit is 1 when nUARTCTS is LOW. | RO | - |
...
![Page 977 figure](images/fig_p0977.png)
...
the PEN bit disables parity checking and generation. | RW | 0x0 |
...
bit has no effect when the PEN bit disables parity checking and generation. | RW | 0x0 |
| 1 | PEN: Parity enable: 0 = parity is disabled and no parity bit added to the data
frame 1 = parity checking and generation is enabled. | RW | 0x0 |
...
at least two complete frames. For normal use, this bit must be cleared to 0. | RW | 0x0 |
...
![Page 978 figure](images/fig_p0978.png)
...
receive FIFO for it to be received. | RW | 0x0 |
...
DTE this can be used as Ring Indicator (RI). | RW | 0x0 |
| 12 | OUT1: This bit is the complement of the UART Out1 (nUARTOut1) modem
status output. That is, when the bit is programmed to a 1 the output is 0. For
DTE this can be used as Data Carrier Detect (DCD). | RW | 0x0 |
...
to a 1 then nUARTRTS is LOW. | RW | 0x0 |
...
programmed to a 1 then nUARTDTR is LOW. | RW | 0x0 |
...
middle of reception, it completes the current character before stopping. | RW | 0x1 |
...
depending on the setting of the SIREN bit. When the UART is disabled in the
middle of transmission, it completes the current character before stopping. | RW | 0x1 |
...
the modem inputs. This bit is cleared to 0 on reset, to disable loopback. | RW | 0x0 |
| 6:3 | Reserved. | - | - |
...
![Page 979 figure](images/fig_p0979.png)
...
power, but might reduce transmission distances. | RW | 0x0 |
...
UARTEN bit disables the UART. | RW | 0x0 |
...
either UART signals or SIR signals depending on the setting of the SIREN bit. | RW | 0x0 |
...
/ 8 full b101-b111 = reserved. | RW | 0x2 |
...
becomes <= 7 / 8 full b101-b111 = reserved. | RW | 0x2 |
...
is set. A write of 0 clears the mask. | RW | 0x0 |
...
![Page 980 figure](images/fig_p0980.png)
...
UARTDSRINTR interrupt is set. A write of 0 clears the mask. | RW | 0x0 |
...
UARTDCDINTR interrupt is set. A write of 0 clears the mask. | RW | 0x0 |
...
UARTCTSINTR interrupt is set. A write of 0 clears the mask. | RW | 0x0 |
...
![Page 981 figure](images/fig_p0981.png)
...
![Page 982 figure](images/fig_p0982.png)
...
interrupt state of the UARTCTSINTR interrupt. | RO | - |
| 0 | RIMMIS: nUARTRI modem masked interrupt status. Returns the masked
interrupt state of the UARTRIINTR interrupt. | RO | - |
...
![Page 983 figure](images/fig_p0983.png)
...
r1p3 0x2 r1p4 0x2 r1p5 0x3 | RO | 0x3 |
