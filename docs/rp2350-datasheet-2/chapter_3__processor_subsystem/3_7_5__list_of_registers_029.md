...
the ITM for output to the TPIU | RW | 0x0 |
...
Sample packet generation | RW | 0x1 |
| 11:10 | SYNCTAP: Selects the position of the synchronization packet counter tap on
the CYCCNT counter. This determines the Synchronization packet rate | RW | 0x2 |
...
CYCCNT wraps to zero | RW | 0x00000000 |
...
number of instructions executed, minus one | RW | 0x00 |
...
and Data Address comparators | RW | 0x0 |
| 9:6 | Reserved. | - | - |
| 5:4 | ACTION: Defines the action on a match. This field is ignored and the
comparator generates no actions if it is disabled by MATCH | RW | 0x0 |
...
| 23:12 | Reserved. | - | - |
| 11:10 | DATAVSIZE: Defines the size of the object being watched for by Data Value
and Data Address comparators | RW | 0x2 |
| 9:6 | Reserved. | - | - |
| 5:4 | ACTION: Defines the action on a match. This field is ignored and the
comparator generates no actions if it is disabled by MATCH | RW | 0x2 |
...
interrupt 32*n+m | RW | 0x00000000 |
...
RES0 if the PE does not implement this interrupt | RW | 0x0 |
| 27:24 | Reserved. | - | - |
...
1 SLEEPDEEP is only available to Secure state | RW | 0x0 |
...
at a requested priority less than 0 | RW | 0x0 |
...
attempting to perform integer division by zero | RW | 0x0 |
...
pend interrupts via the STIR | RW | 0x0 |
...
states, `IAAMO the pending state of the UsageFault exception `FTSSS | RW | 0x0 |
...
of the HardFault exception `FTSSS | RW | 0x0 |
...
present error has occurred | RW | 0x0 |
...
priority, or because it was disabled | RW | 0x0 |
| 29:2 | Reserved. | - | - |
| 1 | VECTTBL: Indicates when a fault has occurred because of a vector table read
error on exception processing | RW | 0x0 |
| 0 | Reserved. | - | - |
...
set, otherwise it is UNKNOWN | RW | 0x00000000 |
...
valid only when BFSR.BFARVALID is set, otherwise it is UNKNOWN | RW | 0x00000000 |
...
the implemented Synchronization Primitive instructions | RO | 0x5 |
...
the implemented Synchronization Primitive instructions | RO | 0x3 |
...
memory location in it modified | RO | 0x0 |
...
data caches and unified caches that are controlled by the PE | RO | 0x0 |
| 15:14 | RES1_1: Reserved, RES1 | RO | 0x3 |
| 13:4 | Reserved. | - | - |
| 3:0 | IMINLINE: Log2 of the number of words in the smallest cache line of all the
instruction caches that are controlled by the PE | RO | 0x0 |
...
programmed to the same value as the CP10 field, then the value is UNKNOWN | RW | 0x0 |
...
NMIs, and exception handlers when FAULTMASK is set to 1 | RW | 0x0 |
...
to be checked against | RW | 0x0000000 |
| 4 | Reserved. | - | - |
...
marked as Non-secure or Secure | RW | 0x0 |
...
instruction from this region | RW | 0x0 |
...
bit can be cleared by other exceptions, such as BusFault | RW | 0x0 |
| 5 | LSPERR: Stick flag indicating that an SAU or IDAU violation occurred during
the lazy preservation of floating-point state | RW | 0x0 |
...
Secure to Non-secure memory | RW | 0x0 |
...
preservation. See LSPERR | RW | 0x0 |
...
being set to 1 when returning from an exception in the Non-secure state | RW | 0x0 |
| 1 | INVIS: This bit is set if the integrity signature in an exception stack frame is
found to be invalid during the unstacking operation | RW | 0x0 |
...
register, which is not accessible to the Non-secure state | RW | 0x00000000 |
...
DHCSR.C_HALT from 1 to 0, or an External Restart Request | RO | 0x0 |
...
mask PendSV, SysTick and external configurable interrupts | RW | 0x0 |
...
FP register to transfer | RW | 0x00 |
...
registers, special-purpose registers, and Floating-point Extension registers | RW | 0x00000000 |
...
secure state and whether debug events are allowed in Secure state | RO | 0x0 |
...
a state information error, for example an Undefined Instruction exception | RW | 0x0 |
| 6 | VC_CHKERR: Enable halting debug trap on a UsageFault exception caused by
a checking error, for example an alignment check error | RW | 0x0 |
| 5 | VC_NOCPERR: Enable halting debug trap on a UsageFault caused by an
access to a coprocessor | RW | 0x0 |
...
are accessed to the debugger | RW | 0x0 |
...
the CONTROL.FPCA bit to 1 | RW | 0x0 |
...
lazy floating-point state preservation behavior | RW | 0x0 |
...
behaves as RAZ/WI when accessed from the Non-secure state | RW | 0x0 |
...
floating-point stack frame was able to set the BusFault exception to pending | RW | 0x1 |
...
floating-point stack frame was able to set the HardFault exception to pending | RW | 0x1 |
...
activated, or when a floating-point instruction is executed | RW | 0x0 |
| 1 | USER: Indicates the privilege level of the software executing when the PE
allocated the floating-point stack frame | RW | 0x1 |
...
allocated on an exception stack frame | RW | 0x00000000 |
| 2:0 | Reserved. | - | - |
...
instruction trace stream when the selected event occurs | RW | 0x0 |
...
balanced against the risk of a FIFO overflow | RW | 0x0 |
| 1:0 | Reserved. | - | - |
...
synchronization period of 2^20 bytes | RO | 0x0a |
...
enabled for the corresponding exception level | RW | 0x0 |
| 18:17 | Reserved. | - | - |
...
counter each time the reload event occurs | RW | 0x0000 |
...
implemented on the lower register for a pair of resource selectors | RW | 0x0 |
...
provided per resource from the group | RW | 0x00 |
...
TRCSSCCRn.SAC or TRCSSCCRn.ARC selects, have matched | RW | 0x0 |
| 30:4 | Reserved. | - | - |
...
comparator input 1 for Single-shot control | RW | 0x0 |
...
of the field for each channel output | RO | 0x0 |
