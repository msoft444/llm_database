...
![Page 367 figure](images/fig_p0367.png)
...
![Page 368 figure](images/fig_p0368.png)
...
| Boot type is RAM IMAGE
_ | Make note for later: this requests a scan of a RAM region for a preloaded image. |
| Boot type is FLASH UPDATE
_ | Make note for later: modifies some flash boot behaviour, as described in Section 5.1.16. |
...
![Page 369 figure](images/fig_p0369.png)
...
RAM IMAGE
_ | Go to Check BOOTSEL. |
| BOOT_FLAGS0.DISABLE_SR
AM_WINDOW_BOOT is set | Go to Check BOOTSEL. |
...
BOOT_FLAGS0 | Go to Prepare for BOOTSEL. |
| Always | Issue XIP exit sequence to chip select 0. |
| FLASH_DEVINFO has GPIO
and size for chip select 1 | Issue XIP exit sequence to chip select 1. |
...
![Page 370 figure](images/fig_p0370.png)
...
permitted. |
| Both USB interfaces
disabled | Go to Boot Failure. |
| Otherwise | Outcome: Enter USB bootloader. The bootloader reboots if a UF2 image is downloaded,
marking a FLASH UPDATE in the watchdog scratch registers if applicable, and the boot path
_
...
downloaded, with a RAM IMAGE boot type, and the boot path restarts from Entry. Valid
_
...
![Page 371 figure](images/fig_p0371.png)
