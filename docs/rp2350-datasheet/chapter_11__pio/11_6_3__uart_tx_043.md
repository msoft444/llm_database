...
*Figure 58. UART serial Bit Clock format. The line is TX high when idle. The State transmitter pulls the line down for one bit period to signify the start of a serial frame (the "start bit"), and a small, fixed number of data bits follows. The line returns to the idle state for at least one 8 .program uart_tx bit period (the "stop 9 .side_set 1 opt bit") before the next*
