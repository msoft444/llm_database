...
*Figure 49. Execution System Clock of manual_pull Instruction program. X is used as Scratch X a loop counter. On each iteration, one Clock pin (side -set) data bit is shifted out, and the clock is OSR shift count asserted low, then high. A delay cycle on each instruction brings the total up to four cycles per iteration. After the third loop, a fourth bit is shifted out, and the state machine loop counter. immediately returns to the start of the*
...
*Figure 50. Execution clock of auto_push_pull Current Instruction program. The state Stall machine stalls on an OUT until data has TX FIFO Empty travelled through the TX FIFO Pop TX FIFO into the OSR. OSR Count (0=full) Subsequently, the OSR is refilled RX FIFO Push simultaneously with ISR Count (0=empty) each OUT operation RX FIFO Push (due to bit count of 32), and IN data bypasses the ISR and goes straight to the RX FIFO. The state machine stalls again when the FIFO has drained, and the OSR is once again empty.*
