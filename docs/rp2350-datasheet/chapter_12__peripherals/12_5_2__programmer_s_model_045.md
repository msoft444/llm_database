...
*Table 1130. Mapping of PWM channels to GPIO pins on RP2350. This is also shown in*
...
*Figure 112. The Input (Count) Count*

![Page 1080 figure](images/fig_p1080_1.png)
...
![Page 1080 figure](images/fig_p1080_2.png)
...
*Figure 114. In phase- Input (Count)*

![Page 1081 figure](images/fig_p1081_1.png)
...
![Page 1081 figure](images/fig_p1081_2.png)
...
*Figure 115. Glitch-free 0% duty cycle output for CC = 0, and glitch- TOP free 100% duty cycle output for CC = TOP + 1 0 IOVDD 0*
...
*Figure 116. The input Input (Count)*

![Page 1082 figure](images/fig_p1082_1.png)
...
![Page 1082 figure](images/fig_p1082_2.png)
...
*Figure 117. The input value changes whilst Count the counter is mid- TOP ramp. This produces additional toggling at 2×TOP/3 the output. TOP/3 0 V IOVDD 0 next wrap.*
...
*Figure 118. Each Counter at top counter wrap causes IRQ the interrupt request signal to assert. The CC_A processor enters its interrupt handler, writes to its copy of the CC register, and clears the interrupt. When the counter wraps again, the latched version of the CC register is instantaneously updated with the most recent value written by software, and this*
...
*Figure 119. The clock divider generates an 1 DIV_INT enable signal. The .0 DIV_FRAC counter only counts on cycles where this Counter enable signal is high. A clock divisor of 1 causes the 3 DIV_INT*
...
*Figure 121. The clock enable signal, output Clock by the clock divider, controls the rate of counting. Phase DIV_INT advance forces the Clock enable clock enable high on Count cycles where it is low, causing the counter to jump forward by one DIV_INT count. Phase retard forces the clock CSR_PH_ADV enable low when it Clock enable would be high, holding the counter back by one count. DIV_INT CSR_PH_ADV Clock enable*
