...
*Figure 8. An interpolator. The two accumulator registers and three base registers have single- Result 0 cycle read/write access from the Result 1 processor. The interpolator is organised into two lanes, which perform masking, shifting and sign-extension Result 0 operations on the two accumulators. This Result 1 produces three possible results, by adding the intermediate shift/mask values to the three base registers. From left to right, the multiplexers on each lane are controlled by the ACCUMx_ADD register. following flags in the CTRL registers: CROSS_RESULT, CROSS_INPUT, SIGNED, and ADD_RAW.*
