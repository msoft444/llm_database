...
Workaround
Add a single block at the start of the UF2 with an Absolute family ID, targeting the end of Flash, with block
...
RP2350 A2
Description
When the block loop of a binary contains an IMAGE_DEF that is explicitly invalid before the valid IMAGE_DEF for
...
Workaround
Instead of an explicitly invalid IMAGE_DEF, use an IGNORED item. RP2040 doesn’t require an IMAGE_DEF to boot a
