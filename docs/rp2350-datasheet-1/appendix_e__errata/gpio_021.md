...
Description
The CHAN_ABORT register commands a DMA channel to stop issuing transfers, and to clear its BUSY flag
...
Workaround
Before aborting an active channel, clear the EN bit (CH0_CTRL_TRIG.EN) of both the aborted channel and
...
write since.
Workaround
Don’t use CHAIN_TO in conjunction with zero-length transfers. Avoid zero-length transfers in the middle of
